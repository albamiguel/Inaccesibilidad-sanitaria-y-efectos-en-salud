...
#Obtenemos el núemro de columnas que tiene:
...
# Seleccionamos sólo aquellas variables que nos interesan: Sí o no / Total
Correlacion_transporte_inaccesibilidad <-
...
Correlacion_transporte_inaccesibilidad
table(Correlacion_transporte_inaccesibilidad$`Sí o no`)

#

Resultado_final_transporte <- Correlacion_transporte_inaccesibilidad %>% 
  mutate(`Sí o no` = factor(`Sí o no`, labels = c("No", "No", "Sí", "Total"))) %>% 
  filter(`Sí o no` != "Total")
...
Resultado_final_transporte %>% 
  drop_na() %>% 
  ggplot(data = ., mapping = aes(x = `Sí o no` , y = `Comunidad autónoma` )) +
  geom_point(na.rm = TRUE)
