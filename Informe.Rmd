...
- **Total**: compuesto por 1200 elementos clase caracter. Es el recuento total de cada variable distinta. 
...
Datos_filtrados_economicos %>% 
  ...
  labs(title= "Inaccesibilidad por motivos economicos", x= " ", y= " ",fill="Tipos de atencion sanitaria")+
...
**2) Queremos ver las diferencias entre hombres y mujeres en cuanto a inaccesibilidada por transporte en el total nacional**
...
Datos_filtrados2 %>%
  drop_na() %>%
  ggplot(data = .,aes(x=`Sexo`, y= Total, color= "Red")) +
  geom_point()+
  xlab(" ")+
  ylab(" ")+
  ggtitle("Inaccesibilidad por sexos")+
  theme_minimal()
...
En el gráfico anterior se muestran las personas que han sufrido inaccesibilidad o demora excesiva a la asistencia sanitaria debido al transporte o la distancia.
La comparación que hemos realizado es por sexos. Podemos observar que hay una diferencia entre sexos. Las mujeres han sufrido mayor inaccesibilidad que los hombres.