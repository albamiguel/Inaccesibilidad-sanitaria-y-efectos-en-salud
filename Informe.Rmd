...
Resultado3 <-
  ...
  geom_bar(stat='identity',fill =rainbow(19))+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust=1))+
  theme(axis.text.y = element_text(angle = 0, vjust = 0, hjust=1))+
...
Resultado4 <-
  drop_na() %>%
#mutate(`Comunidad autónoma`= fct_reorder(`Comunidad autónoma`, Total)%>%
  ggplot(data = Datos_filtrados,
       mapping = aes(reorder(`Comunidad autónoma`,Total), Total)) +
